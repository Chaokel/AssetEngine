...
void OpenGLFunctions::RenderTerrain(Terrain* ter)
{
	float scale = 5.0f / std::max(ter->GetWidth() - 1, ter->GetLength() - 1);
	glScalef(scale, scale, scale);
	glTranslatef(-(float)(ter->GetWidth() - 1) / 2,
		0.0f,
		-(float)(ter->GetLength() - 1) / 2);
